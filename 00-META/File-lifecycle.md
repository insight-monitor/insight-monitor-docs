...
`status: raw`, `type`, `audience` and `domain`.
Firstly created documents don't need a strict structure, if you need to do a git commit, add `status(raw)`
## Optimization
...
3. When done, change status `status: in-review` commit and create a [[Git-issue|git issue]] for everyone or the one responsible to review
   If you are the responsible, create the issue nevertheless, to create a throughout git history.
4. When reviewed, change `status: completed` and make a [[Pull-request-template|PR]] to main. 
5. If deprecated, change `status: deprecated` and move to 98-DEPRECATED folder.
## Deletion
If the files were on main and were valid, DO NOT delete them, put them into 98-DEPRECATED.
