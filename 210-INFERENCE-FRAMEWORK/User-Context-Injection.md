...
User-defined context is serialized into a structured block appended to the
system prompt before every inference cycle.
...
## Context freshness
User context is cached and reused. When objectives change or projects shift,
the user must update their context to maintain inference accuracy. Stale context
(calling a past project as current) degrades classification quality.